...
export default function maxWidth(node) {
  ...
  /* 
    Use a BFS to count the number of children in each layer.
    Keep track of which layer each node is in, and add 1 
    to the layer when adding a child into the queue. 

    visitCallback = increment the index of an array corresponding 
    to that layer by 1. 
